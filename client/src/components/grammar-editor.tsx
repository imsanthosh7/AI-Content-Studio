...
export function GrammarEditor({ 
  ...
  return (
    <>
      <div className="bg-surface rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-medium text-secondary">Text Editor</h2>
            <div className="flex items-center space-x-3">
              <span data-testid="text-word-count" className="text-sm text-gray-500">{wordCount} words</span>
              <button 
                onClick={onClearText}
        ...
        <div className="p-6">
          <textarea 
            ...
            className="w-full h-96 p-4 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-primary focus:border-transparent text-base leading-relaxed"
          ...
          <div className="mt-4 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <button 
                ...
                className="border border-gray-300 hover:bg-gray-50 text-secondary px-6 py-2.5 rounded-lg font-medium transition-colors flex items-center space-x-2"
              >
      ...
      {/* Correction History */}
      <div className="mt-6 bg-surface rounded-xl shadow-sm border border-gray-200">
        <div className="border-b border-gray-200 px-6 py-4">
          <h3 className="text-lg font-medium text-secondary">Correction History</h3>
        </div>
        <div className="p-6">
          {correctionHistory && correctionHistory.length > 0 ? (
                ...
                <div key={correction.id} className="space-y-2">
                  {correction.suggestions.map((suggestion: any, index: number) => (
                    <div key={index} className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <span className="text-xs text-gray-400 flex-shrink-0">{suggestion.confidence}%</span>
                    </div>
                  ))}
                </div>
              ))}
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500">No correction history yet</p>
            </div>
          )}
        </div>
      </div>
