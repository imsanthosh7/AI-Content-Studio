...
const PLATFORM_GUIDELINES = {
  linkedin: {
    maxLength: 3000,
    style:
      "Professional networking platform. Use hashtags strategically. Focus on career insights, industry news, professional achievements.",
    format: "Can include line breaks, bullet points, and professional hashtags",
  },
  instagram: {
    maxLength: 2200,
    style:
      "Visual storytelling platform. Use engaging hashtags. Focus on lifestyle, behind-the-scenes, visual content.",
    format: "Can include emojis, trending hashtags, and engaging captions",
  },
  twitter: {
    maxLength: 280,
    style:
      "Concise, engaging microblogging. Use trending hashtags. Be direct and impactful.",
    format:
      "Must be under 280 characters. Include relevant hashtags and mentions",
  },
  reddit: {
    maxLength: 3500,
    style:
      "Authentic, conversational community discussion. Use Reddit culture and terminology. Be genuine and add value.",
...
async function generateContent(
  ...
  switch (contentType) {
    ...
    case "comment-reply":
      if (!platform)
        throw new Error("Platform is required for comment replies");
    ...
    const response = await ai.models.generateContent({
      ...
      config:
        contentType === "grammar"
          ? {
              systemInstruction:
                "You are a grammar correction tool. Return only the corrected text with no explanations, quotes, or additional commentary.",
            }
          : undefined,
    });
...
function generateSocialMediaPrompt(
  text: string,
  platform: keyof typeof PLATFORM_GUIDELINES,
  mood?: MoodType,
  charLimit?: number
): string {
...
function generateCommentReplyPrompt(
  text: string,
  platform: Platform,
  mood?: MoodType
): string {
  ...
  const platformContext =
    platform === "reddit" ? "Reddit comment or post" : `${platform} comment`;
...
${
  platform === "reddit"
    ? "- Use Reddit's conversational style and terminology"
    : ""
}
